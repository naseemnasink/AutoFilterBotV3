    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            if int(index) == 1:
                ...
                buttons.append(
                    [InlineKeyboardButton("◉ ɴᴇxjᴛ ᴘᴀɢᴇ ◉", callback_data=f"next_{int(index)-1}_{keyword}")]
                )
                buttons.append(
                ...
                buttons.append(
                    [InlineKeyboardButton("◉ ʙᴀᴄᴋ ᴘᴀɢᴇ ◉", callback_data=f"back_{int(index)-1}_{keyword}"),InlineKeyboardButton("◉ ɴᴇuxᴛ ᴘᴀɢᴇ ◉", callback_data=f"next_{int(index)-1}_{keyword}")]
                )
                buttons.append(
                    [InlineKeyboardButton(f"◉ ᴘᴀɢᴇ ɴᴜᴍʙᴇʀꜱ ◉ ⟨{int(index)} - {data['total']}⟩", callback_data="pages")]
                )
